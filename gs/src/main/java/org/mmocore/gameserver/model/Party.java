...
public class Party implements PlayerGroup
{
	...
	public boolean addPartyMember(Player player)
	{
		...
		for(Player member : _members)
			if(member != player)
			{
				List<L2GameServerPacket> pmember = new ArrayList<L2GameServerPacket>(addInfo.size() + 4);
		...
		if(player.getServitor() != null)
			player.getServitor().broadcastCharInfoImpl(this);
	...
	public boolean removePartyMember(Player player, boolean kick, boolean withdrawal)
	{
		...
		boolean dissolve = isLeader && withdrawal; // Лидер добровольно вышел, пати распускается независимо от количества
		...
		for(Player member : _members)
		{
			...
			pmember.add(new RelationChanged().add(player, member));
			if(member.getServitor() != null)
				member.getServitor().broadcastCharInfoImpl(player);
	...
	private void updateLeaderInfo()
	{
		...
		SystemMessage msg = new SystemMessage(SystemMsg.C1_HAS_BECOME_THE_PARTY_LEADER).addName(leader);
	...
	private void distributeAdena(Player player, ItemInstance item, NpcInstance fromNpc)
	{
		...
		for(Player member : membersInRange)
		{
			...
			member.sendPacket(SystemMessage.obtainItems(ItemTemplate.ITEM_ID_ADENA, count, 0));
		}
	...
	public void requestLootChange(byte type)
	{
		...
		SystemMessage sm = new SystemMessage(SystemMsg.REQUESTING_APPROVAL_FOR_CHANGING_PARTY_LOOT_TO_S1);
	...
	private synchronized void finishLootRequest(boolean success)
	{
		...
		if(success)
		{
			...
			SystemMessage sm = new SystemMessage(SystemMsg.PARTY_LOOT_WAS_CHANGED_TO_S1);
