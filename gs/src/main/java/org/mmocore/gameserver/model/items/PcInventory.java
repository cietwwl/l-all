...
import org.mmocore.gameserver.model.items.listeners.AccessoryListener;
import org.mmocore.gameserver.model.items.listeners.ArmorSetListener;
import org.mmocore.gameserver.model.items.listeners.BowListener;
import org.mmocore.gameserver.model.items.listeners.ItemAugmentationListener;
import org.mmocore.gameserver.model.items.listeners.ItemEnchantOptionsListener;
import org.mmocore.gameserver.model.items.listeners.ItemSkillsListener;
import org.mmocore.gameserver.network.l2.components.SystemMsg;
import org.mmocore.gameserver.network.l2.s2c.ExBR_AgathionEnergyInfo;
...
public class PcInventory extends Inventory
{
	 ...
	 * Снимает все вещи, которые нельзя носить, включает-выключает руны.
	...
	public void validateItems()
	{
		...
		for (ItemInstance item : _items)
			if (item != null && item.getItemType() == EtcItemType.RUNE)
				...
				{
					if (!item.getTemplate().testCondition(getActor(), item, false))
					{
						item.setEquipped(false); // Выключаем руну
				...
				else
				{
					if (item.getTemplate().testCondition(getActor(), item, false))
					{
						item.setEquipped(true); // Включаем руну
	...
	public void sort(int[][] order)
	{
		...
		if (needSort)
			CollectionUtils.shellSort(_items, ItemOrderComparator.getInstance());
	}
	...
	public boolean isLockedItem(int itemId)
	{
		switch (_lockType)
		{
			case INCLUDE:
				return  ArrayUtils.contains(_lockItems, itemId);
			case EXCLUDE:
				return !ArrayUtils.contains(_lockItems, itemId);
			default:
				return false;
	...
	protected void onRestoreItem(ItemInstance item)
	{
		...
		if (item.getItemType() == EtcItemType.RUNE && item.getTemplate().testCondition(getActor(), item, false))
		{
			item.setEquipped(true); // Активируем руну только если это возможно
			_listeners.onEquip(-1, item);
		}
	...
	protected class LifeTimeTask extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			...
			if(left <= 0)
				player.sendPacket(new SystemMessage(SystemMsg.THE_LIMITEDTIME_ITEM_HAS_DISAPPEARED_BECAUSE_THE_REMAINING_TIME_RAN_OUT));
		}
	}
}