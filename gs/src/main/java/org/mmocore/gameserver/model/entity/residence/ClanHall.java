...
import org.mmocore.gameserver.dao.ClanDataDAO;
import org.mmocore.gameserver.dao.ClanHallDAO;
...
public class ClanHall extends Residence
{
	...
	public ClanHall(StatsSet set)
	{
		...
		_rentalFee = (long) (set.getLong("rental_fee", 0) * Config.CLANHALL_LEASE_MULTIPLIER);
		_minBid = set.getLong("min_bid", 0);
		_deposit = set.getLong("deposit", 0);
	}
	...
	public void init()
	{
		...
		if (Config.ALT_NOT_ALLOW_TW_WARDS_IN_CLANHALLS && getSiegeEvent().getClass() != ClanHallAuctionEvent.class)
		...
		if(getPaidCycle() >= REWARD_CYCLE)
		{
			...
			else
			{
				...
				if(member.isOnline())
					...
					PlayerMessageStack.getInstance().mailto(member.getObjectId(), SystemMsg.THE_CLAN_HALL_FEE_IS_ONE_WEEK_OVERDUE_THEREFORE_THE_CLAN_HALL_OWNERSHIP_HAS_BEEN_REVOKED);
