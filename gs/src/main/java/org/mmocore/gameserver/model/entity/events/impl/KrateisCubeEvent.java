...
import org.mmocore.gameserver.network.l2.s2c.ExPVPMatchCCMyRecord;
import org.mmocore.gameserver.network.l2.s2c.ExPVPMatchCCRecord;
import org.mmocore.gameserver.network.l2.s2c.ExPVPMatchCCRetire;
...
public class KrateisCubeEvent extends Event
{
	...
	private void giveEffects(Player player)
	{
		...
		{
			SkillEntry skill = SkillTable.getInstance().getSkillEntry(SKILL_IDS[j], SKILL_LEVEL[j]);
			if(skill != null)
	...
	{
		KrateisCubeEvent cubeEvent = target.getEvent(KrateisCubeEvent.class);
		if(cubeEvent != null)
		{
			if(!quiet)
