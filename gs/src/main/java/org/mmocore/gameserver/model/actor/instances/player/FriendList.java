...
import org.mmocore.gameserver.dao.CharacterFriendDAO;
import org.mmocore.gameserver.model.GameObjectsStorage;
import org.mmocore.gameserver.model.Player;
import org.mmocore.gameserver.model.World;
import org.mmocore.gameserver.network.l2.components.SystemMsg;
import org.mmocore.gameserver.network.l2.s2c.L2Friend;
import org.mmocore.gameserver.network.l2.s2c.L2FriendStatus;
...
public class FriendList
{
	...
	public void removeFriend(String name)
	{
		...
		if(objectId > 0)
		{
			...
			_owner.sendPacket(new SystemMessage(SystemMsg.S1_HAS_BEEN_REMOVED_FROM_YOUR_FRIENDS_LIST).addString(name), new L2Friend(name, false, friendChar != null, objectId));

			if(friendChar != null)
				friendChar.sendPacket(new SystemMessage(SystemMsg.S1_HAS_BEEN_DELETED_FROM_YOUR_FRIENDS_LIST).addName(_owner), new L2Friend(_owner, false));
		}
		else
			_owner.sendPacket(new SystemMessage(SystemMsg.C1_IS_NOT_ON_YOUR_FRIEND_LIST).addString(name));
	}
	...
	public void notifyFriends(boolean login)
	{
		for(Friend friend : _friendList.values())
		{
			...
			if(friendPlayer != null)
			{
				...
				if(login)
					friendPlayer.sendPacket(new SystemMessage(SystemMsg.YOUR_FRIEND_S1_JUST_LOGGED_IN).addName(_owner));
