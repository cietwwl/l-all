...
public final class EnchantSkillLearn
{
	...
	public EnchantSkillLearn(int id, int lvl, String name, String type, int minSkillLvl, int baseLvl, int maxLvl, int importance)
	{
		_id = id;
		_level = lvl;
		_baseLvl = baseLvl;
	...
	public int[] getCost()
	{
		return SkillTable.getInstance().getSkillEntry(_id, 1).getTemplate().isOffensive() ? _priceCombat[_level % 100] : _priceBuff[_level % 100];
	}
