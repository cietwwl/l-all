...
import org.mmocore.gameserver.network.l2.s2c.ExShowQuestMark;
import org.mmocore.gameserver.network.l2.s2c.PlaySound;
import org.mmocore.gameserver.network.l2.s2c.QuestList;
import org.mmocore.gameserver.network.l2.s2c.SystemMessage;
import org.mmocore.gameserver.network.l2.s2c.TutorialEnableClientEvent;
import org.mmocore.gameserver.network.l2.s2c.TutorialShowHtml;
import org.mmocore.gameserver.network.l2.s2c.TutorialShowQuestionMark;
...
public final class QuestState
{
			...
			switch(_quest.getParty())
			{
				...
				case Quest.PARTY_ALL:
					...
					else
					{
						...
						for(Player temp : actorPlayer.getParty().getPartyMembers())
							if(temp.isInRange(actorPlayer, Config.ALT_PARTY_DISTRIBUTION_RANGE))
								players.add(temp);
					}
					break;
				default:
	...
	public QuestState exitCurrentQuest(boolean repeatable)
	{
		...
		{ // Otherwise, delete variables for quest and update database (quest CANNOT be created again => not repeatable)
			...
			CharacterQuestDAO.getInstance().replace(this); // FIXME: оно вроде не нужно?
		}
	...
	public long takeItems(int itemId, long count)
	{
		...
		if(count < 0 || count > item.getCount()) //TODO [G1ta0] analyze this
			count = item.getCount();
