...
public class ClanAirShipControllerInstance extends AirShipControllerInstance
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		if(command.equalsIgnoreCase("summon"))
		{
			...
			{
				ChatUtils.shout(this, NpcString.IN_AIR_HARBOR_ALREADY_AIRSHIP_DOCKED_PLEASE_WAIT_AND_TRY_AGAIN);
				return;
			}

			if(!ItemFunctions.deleteItem(player, ENERGY_STAR_STONE, 5))
			{
				player.sendPacket(new SystemMessage(SystemMsg.AN_AIRSHIP_CANNOT_BE_SUMMONED_BECAUSE_YOU_DONT_HAVE_ENOUGH_S1).addItemName(ENERGY_STAR_STONE));
				return;
			}
			...
			ChatUtils.shout(this, NpcString.AIRSHIP_IS_SUMMONED_IS_DEPART_IN_5_MINUTES);
		}
		else if(command.equalsIgnoreCase("register"))
		{
			...
			if(ItemFunctions.getItemCount(player, AIRSHIP_SUMMON_LICENSE) == 0)
			{
				player.sendPacket(SystemMsg.YOU_DO_NOT_HAVE_ENOUGH_REQUIRED_ITEMS);
				return;
			}

			ItemFunctions.deleteItem(player, AIRSHIP_SUMMON_LICENSE, 1);
