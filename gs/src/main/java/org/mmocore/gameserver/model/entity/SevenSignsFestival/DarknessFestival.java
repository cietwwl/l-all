...
public class DarknessFestival extends Reflection
{
	...
	public void collapse()
	{
		...
		Player leader = getParty() != null ? getParty().getPartyLeader() : null;
		if(SevenSigns.getInstance().getCurrentPeriod() == SevenSigns.PERIOD_COMPETITION && leader != null && leader.getReflection() == this)
		{
			ItemInstance bloodOfferings = leader.getInventory().getItemByItemId(SevenSignsFestival.FESTIVAL_BLOOD_OFFERING);
			...
			if(leader.getInventory().destroyItem(bloodOfferings))
			{
				...
				leader.sendPacket(new SystemMessage(SystemMsg.YOUR_CONTRIBUTION_SCORE_HAS_INCREASED_BY_S1).addNumber(offeringScore));
				...
				leader.sendMessage(new CustomMessage("l2p.gameserver.model.instances.L2FestivalGuideInstance.BloodOfferings"));
		}
	...
	{
		CustomMessage cm = new CustomMessage(s);
		for(Player p : getPlayers())
			p.sendMessage(cm);
	}
