...
public class Hero
{
	...
	public void activateHero(Player player)
	{
		...
		if(player.getClan() != null && player.getClan().getLevel() >= 5)
		{
			...
			player.getClan().broadcastToOtherOnlineMembers(new SystemMessage(SystemMsg.CLAN_MEMBER_C1_WAS_NAMED_A_HERO_S2_POINTS_HAVE_BEEN_ADDED_TO_YOUR_CLANS_REPUTATION_SCORE).addName(player).addNumber(Math.round(1000 * Config.RATE_CLAN_REP_SCORE)), player);
		}
		player.broadcastUserInfo(true);
	...
	public void showHeroDiary(Player activeChar, int heroclass, int charid, int page)
	{
		...
		if(mainlist != null)
		{
			...
			html.replace("%message%", StringUtils.defaultString(_heroMessage.get(charid)));
