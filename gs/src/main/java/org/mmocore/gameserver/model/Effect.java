...
import org.mmocore.gameserver.network.l2.s2c.AbnormalStatusUpdate;
import org.mmocore.gameserver.network.l2.s2c.ExOlympiadSpelledInfo;
import org.mmocore.gameserver.network.l2.s2c.PartySpelled;
import org.mmocore.gameserver.network.l2.s2c.ShortBuffStatusUpdate;
...
public abstract class Effect extends RunnableImpl implements Comparable<Effect>, FuncOwner
{
	...
	protected void onExit()
	{
		...
		if(getEffected().isPlayer() && !getSkill().getTemplate().canUseTeleport() && getEffected().getPlayer().isTeleportBlocked())
	...
	public final void runImpl() throws Exception
	{
		...
		if(setState(FINISHING, FINISHED))
		{
			...
			if(getSkill().getTemplate().getDelayedEffect() > 0)
				SkillTable.getInstance().getSkillEntry(getSkill().getTemplate().getDelayedEffect(), getSkill().getTemplate().getDelayedEffectLevel()).getEffects(_effector, _effected, false, false);
	...
	public boolean maybeScheduleNext(Effect newEffect)
	{
		...
		else if (newEffect.getSkill().getTemplate().isAbnormalInstant())
	...
	{
		if(!isActive() || isHidden() || _skill.getTemplate().isToggle() || _skill.getTemplate().isHandler())
			return;
		//int duration = _skill.getTemplate().isToggle() ? AbnormalStatusUpdate.INFINITIVE_EFFECT : getTimeLeft();
	...
	{
		return _template.isSaveable(getSkill().getTemplate().isSaveable()) && getTimeLeft() >= Config.ALT_SAVE_EFFECTS_REMAINING_TIME;
	}
