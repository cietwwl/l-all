...
public final class PetBabyInstance extends PetInstance
{
	...
	class ActionTask extends RunnableImpl
	{
		...
		{
			SkillEntry skill = onActionTask();
			_actionTask = ThreadPoolManager.getInstance().schedule(new ActionTask(), skill == null ? 1000 : skill.getTemplate().getHitTime() * 333 / Math.max(getMAtkSpd(), 1) - 100);
		}
	}
