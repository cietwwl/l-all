	...
	public Map.Entry<String, String> toString(Player player)
	{
		...
		switch(_id)
		{
			case ACTION_RAID_KILLED:
				message = new CustomMessage("l2p.gameserver.model.entity.Hero.RaidBossKilled").addString(HtmlUtils.htmlNpcName(_param));
				break;
			case ACTION_HERO_GAINED:
				message = new CustomMessage("l2p.gameserver.model.entity.Hero.HeroGained");
				break;
			case ACTION_CASTLE_TAKEN:
				message = new CustomMessage("l2p.gameserver.model.entity.Hero.CastleTaken").addString(HtmlUtils.htmlResidenceName(_param));
				break;
			default:
				return null;
		}
