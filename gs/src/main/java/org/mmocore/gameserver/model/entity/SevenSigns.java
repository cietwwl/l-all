...
import org.mmocore.gameserver.instancemanager.AutoSpawnManager.AutoSpawnInstance;
import org.mmocore.gameserver.instancemanager.RaidBossSpawnManager;
import org.mmocore.gameserver.listener.GameListener;
import org.mmocore.gameserver.listener.game.OnSSPeriodListener;
...
import org.mmocore.gameserver.network.l2.s2c.SSQInfo;
...
public class SevenSigns
{
	...
	public void sendCurrentPeriodMsg(Player player)
	{
		switch(_activePeriod)
		{
			case PERIOD_COMP_RECRUITING:
				...
				return;
			case PERIOD_COMPETITION:
				player.sendPacket(SystemMsg.SEVEN_SIGNS_THE_QUEST_EVENT_PERIOD_HAS_BEGUN_SPEAK_WITH_A_PRIEST_OF_DAWN_OR_DUSK_PRIESTESS_IF_YOU_WISH_TO_PARTICIPATE_IN_THE_EVENT);
				return;
			case PERIOD_COMP_RESULTS:
				player.sendPacket(SystemMsg.SEVEN_SIGNS_QUEST_EVENT_HAS_ENDED_RESULTS_ARE_BEING_TALLIED);
				return;
			case PERIOD_SEAL_VALIDATION:
				player.sendPacket(SystemMsg.SEVEN_SIGNS_THIS_IS_THE_SEAL_VALIDATION_PERIOD_A_NEW_QUEST_EVENT_PERIOD_BEGINS_NEXT_MONDAY);
				return;
		}
	}
	 ...
	 * @param sysMsgId
	 */
	public void sendMessageToAll(IBroadcastPacket msg)
	{
				...
				case CABAL_DAWN:
					switch(getCabalHighestScore())
					{
						...
						case CABAL_DUSK:
							if(duskProportion >= Math.round(0.35 * totalDuskMembers))
								newSealOwner = CABAL_DUSK;
							else if(dawnProportion >= Math.round(0.10 * totalDawnMembers))
								newSealOwner = prevSealOwner;
							else
								newSealOwner = CABAL_NULL;
							break;
		...
		public void runImpl() throws Exception
		{
			...
			switch(periodEnded)
			{
				case PERIOD_COMP_RECRUITING: // Initialization
					sendMessageToAll(SystemMsg.SEVEN_SIGNS_THE_COMPETITION_PERIOD_HAS_BEGUN__VISIT_A_PRIEST_OF_DAWN_OR_PRIESTESS_OF_DUSK_TO_PARTICIPATE_IN_THE_EVENT);
				...
				case PERIOD_COMPETITION: // Results Calculation
					sendMessageToAll(SystemMsg.SEVEN_SIGNS_THE_COMPETITION_PERIOD_HAS_ENDED_THE_NEXT_QUEST_EVENT_WILL_START_IN_ONE_WEEK);
