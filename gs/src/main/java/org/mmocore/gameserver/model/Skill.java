...
import org.mmocore.gameserver.skills.skillclasses.DeathPenalty;
...
public abstract class Skill extends StatTemplate implements Cloneable
{
	...
	protected boolean _isTrigger = false;
	...
	protected Skill(StatsSet set)
	{
		...
		_isSkillTimePermanent = set.getBool("isSkillTimePermanent", _magicType == SkillMagicType.SPECIAL);
		...
		_levelModifier = set.getInteger("levelModifier", 0);
		...
		_skillMastery = set.getEnum("skillMasteryType", SkillMastery.class, isMusic() ? SkillMastery.DOUBLE_TIME : getSkillType().getDefaultSkillMastery());
	...
	public boolean checkCondition(SkillEntry skillEntry, Creature activeChar, Creature target, boolean forceUse, boolean dontMove, boolean first)
	{
		...
		if (first)
		{
			...
			if (isMusic())
			{
				mpConsume2 += activeChar.getEffectList().getActiveMusicCount(getId()) * mpConsume2 / 2;
	...
	public boolean checkPreConditions(SkillEntry skillEntry, Creature activeChar, Creature target)
	{
	...
	public SystemMsg checkTarget(Creature activeChar, Creature target, Creature aimingTarget, boolean forceUse, boolean first)
	{
		...
		if(_isAltUse || _targetType == SkillTargetType.TARGET_UNLOCKABLE || _targetType == SkillTargetType.TARGET_CHEST)
			return null;
		Player player = activeChar.getPlayer();
		// DS: хардкод для саблима
		// Не используем для него обычный набор проверок, достаточно проверки на возможность атаки
		if(player != null && getId() != 1505)
		{
			...
			if(pcTarget != null)
			{
				...
				/*if(player.isInZone(ZoneType.epic) != pcTarget.isInZone(ZoneType.epic)) // DS: зачем это вообще ?
					return SystemMsg.THAT_IS_AN_INCORRECT_TARGET;*/
				...
				if(isOffensive())
				{
					if(target.isPlayer() && target == player)
						return SystemMsg.THAT_IS_AN_INCORRECT_TARGET;
					if(player.isInOlympiadMode() && player.isOlympiadCompStarted() && player.getOlympiadSide() == pcTarget.getOlympiadSide() && !forceUse) // Свою команду атаковать нельзя
						return SystemMsg.THAT_IS_AN_INCORRECT_TARGET;
		...
		if (!forceUse && !isForceUse())
		{
			final boolean autoAttackable = target.isAutoAttackable(activeChar);
	...
	public final Creature getAimingTarget(Creature activeChar, GameObject obj)
	{
		...
		switch(_targetType)
		{
			...
			case TARGET_TUNNEL:
				return target != null && target.isDead() == _isCorpse && !(target == activeChar && isOffensive()) && (!_isUndeadOnly || target.isUndead() && _isBehind == PositionUtils.isBehind(target, activeChar)) ? target : null;
			case TARGET_AREA_AIM_CORPSE:
				return target != null && target.isDead() ? target : null;
			case TARGET_CORPSE:
				if (target == null || !target.isDead())
					return null;
				if (target.isSummon() && target != activeChar.getServitor()) // использовать собственного мертвого самона нельзя
					return target;
        ...
        public List<Creature> getTargets(Creature activeChar, Creature aimingTarget, boolean forceUse, boolean forz)
	{
	...
	public List<Creature> getTargets(Creature activeChar, Creature aimingTarget, boolean forceUse)
	{
		...
		switch(_targetType)
		{
			...
			case TARGET_AREA_CLAN:
			{
				...
				if(player == null)
					break;
				final Player targetPlayer = aimingTarget.getPlayer();
				if (targetPlayer != null)
				{
					final int clanId = targetPlayer.getClan() != null ? targetPlayer.getClanId() : -1;
					addTargetAndPetToList(targets, player, targetPlayer);
					for(Player target : World.getAroundPlayers(aimingTarget, _skillRadius, 600))
					{
						if(target == null || activeChar == target || player == target.getPlayer())
							continue;
						if (target.getClanId() == clanId && checkTarget(activeChar, target, aimingTarget, true, false) != null)
				...
				for(Player target : World.getAroundPlayers(player, _skillRadius, 600))
				{
					...
					switch(_targetType)
					{
						case TARGET_PARTY:
							...
							break;
						case TARGET_CLAN:
							check = (player.getTeam() == TeamType.NONE ? player.getClanId() != 0 && target.getClanId() == player.getClanId() : player.getTeam() == target.getTeam()) || player.getParty() != null && target.getParty() == player.getParty();
							break;
						case TARGET_CLAN_ONLY:
							check = player.getTeam() == TeamType.NONE ? player.getClanId() != 0 && target.getClanId() == player.getClanId() && player.getTeam() == target.getTeam() : player.getTeam() == target.getTeam();
							break;
						case TARGET_ALLY:
							check = player.getTeam() == TeamType.NONE ? player.getClanId() != 0 && target.getClanId() == player.getClanId() || player.getAllyId() != 0 && target.getAllyId() == player.getAllyId() : player.getTeam() == target.getTeam();
							break;
					}
					if(!check)
						continue;
	...
	public final void getEffects(final SkillEntry skillEntry, Creature effector, Creature effected, boolean calcChance, boolean applyOnCaster)
	{
		double timeMult = 1.0;
		...
		getEffects(skillEntry, effector, effected, calcChance, applyOnCaster, 0, timeMult, 0);
	}
	 ...
	 */
	public final void getEffects(final SkillEntry skillEntry, final Creature effector, final Creature effected, final boolean calcChance, final boolean applyOnCaster, final long timeConst, final double timeMult, final int timeFix)
	{
			...
			public void runImpl()
			{
				...
				for (EffectTemplate et : getEffectTemplates())
				{
					...
					for (int i = 0; i < targets.length; i++)
					{
						...
						Env env = new Env(effector, target, skillEntry);

						if(_isReflectable && et._isReflectable && isOffensive() && target != effector && !effector.isTrap())
							if(reflected || Rnd.chance(target.calcStat(isMagic() ? Stats.REFLECT_MAGIC_DEBUFF : Stats.REFLECT_PHYSIC_DEBUFF, 0, effector, skillEntry)))
							{
								if (!reflected)
								{
							...
							else
							{
								...
								if(!et._applyOnCaster && isOffensive() && !isIgnoreResists() && !effector.isRaid())
								{
									...
									if(et.getEffectType().getResistType() != null)
										res += target.calcStat(et.getEffectType().getResistType(), 100., effector, skillEntry) - 100.;
									if(et.getEffectType().getAttributeType() != null)
										res -= effector.calcStat(et.getEffectType().getAttributeType(), 100., target, skillEntry) - 100.;

									res += effected.calcStat(Stats.DEBUFF_RESIST, 100., effector, skillEntry) - 100.;
	...
	public void setIsTrigger(boolean b)
	{
		_isTrigger = b;
	}
	...
	public double getSimpleDamage(SkillEntry skillEntry, Creature attacker, Creature target)
	{
		if(isMagic())
		{
			// магический урон
			double mAtk = attacker.getMAtk(target, skillEntry);
			double mdef = target.getMDef(null, skillEntry);
	...
	/**
	 * @return false если скилл может кастоваться на ходу.
	 * Для altUse-скиллов не используется, они и так кастуются на ходу.
	 */
	...
	/**
	 * Внимание - количество атак вычисляется при первом обращении,
	 * возможны проблемы если скилл еще целиком не загрузился и наличие эффектов неизвестно.
	 */
