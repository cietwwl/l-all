...
public class OlympiadGame
{
	...
	public void winGame(OlympiadTeam winnerTeam, OlympiadTeam looseTeam)
	{
		...
		if(_type != CompType.TEAM)
		{
			...
			if(member1 != null && member2 != null)
			{
				...
				OlympiadHistoryEntry h = new OlympiadHistoryEntry(member1.getObjectId(), member2.getObjectId(), member1.getClassId(), member2.getClassId(), member1.getName(), member2.getName(), _startTime, diff, team, _type.ordinal());
		...
		{
			ItemFunctions.addItem(player, Config.ALT_OLY_BATTLE_REWARD_ITEM, getType().getReward());
			player.sendChanges();
		}
	...
	public void tie()
	{
		...
		if(_type != CompType.TEAM)
		{
			...
			{
				int diff = (int) ((System.currentTimeMillis() - _startTime) / 1000L);
				OlympiadHistoryEntry h = new OlympiadHistoryEntry(member1.getObjectId(), member1.getObjectId(), member1.getClassId(), member2.getClassId(), member1.getName(), member2.getName(), _startTime, diff, 0, _type.ordinal());
	...
	public void broadcastPacket(IBroadcastPacket packet, boolean toTeams, boolean toSpectators)
	{
		if(toTeams)
		{
			_team1.broadcast(packet);
