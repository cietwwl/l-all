		...
		else if(command.startsWith("Festival"))
		{
			...
			switch(val)
			{
				...
				case 2: // Festival 2 xxxx
					...
					{
						player.sendMessage(new CustomMessage("l2p.gameserver.model.instances.L2FestivalGuideInstance.InProgress"));
						return;
					}
					...
					{
						ItemFunctions.deleteItem(p, SevenSignsFestival.FESTIVAL_BLOOD_OFFERING, ItemFunctions.getItemCount(p, SevenSignsFestival.FESTIVAL_BLOOD_OFFERING), false);
					...
					if(!ItemFunctions.deleteItem(player, stoneType, stonesNeeded))
					{
						player.sendMessage(new CustomMessage("l2p.gameserver.model.instances.L2FestivalGuideInstance.NotEnoughSSType"));
						return;
					}
				...
				case 9: // Leave the Festival
					...
					if(playerParty.isLeader(player))
						...
						player.sendMessage("Only party leader can leave festival, if minimum party member is reached.");
					break;
				default:
