...
public class DimensionalRift extends Reflection
{
	...
	public DimensionalRift(Party party, int type, int room)
	{
		...
		for(Player p : party.getPartyMembers())
		{
			...
			if(type >= 11)		// Для DC добавляем игроку лиснер для выброса через минуту после смерти
				p.addListener(_deathListener);
		}
	...
	private class DeathListener implements OnDeathListener
	{
		...
		{
			ThreadPoolManager.getInstance().schedule(new OnDeathTimer(self, DimensionalRift.this), 60000L);
		}
	}

	private class OnDeathTimer extends RunnableImpl
	{
		public Creature _creature;
		public Reflection _ref;

		public OnDeathTimer(Creature creature, Reflection ref)
		{
			_creature = creature;
			_ref = ref;
		}
		...
		public void runImpl() throws Exception
		{
			if(_creature != null && _creature.isDead() && _creature.getReflection() == _ref)
				_creature.teleToLocation(_ref.getReturnLoc(), ReflectionManager.DEFAULT);
		}
	}
}