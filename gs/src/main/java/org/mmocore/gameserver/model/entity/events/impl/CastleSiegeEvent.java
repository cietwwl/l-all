...
public class CastleSiegeEvent extends SiegeEvent<Castle, SiegeClanObject>
{
	public class KillListener implements OnKillListener
	{
		...
		public void onKill(Creature actor, Creature victim)
		{
			...
			if(winner == null || !victim.isPlayer() || winner == victim || !checkIfInZone(victim) || !((Player)victim).isUserRelationActive() || victim.getEvent(CastleSiegeEvent.class) != CastleSiegeEvent.this)
				return;
			...
			{
				if(temp.getEvent(CastleSiegeEvent.class) == null || temp.getLevel() < 40 || !temp.isInRange(winner, Config.ALT_PARTY_DISTRIBUTION_RANGE))
					continue;

				temp.setFame(temp.getFame() + value, CastleSiegeEvent.this.toString());
			}
	...
	public void startEvent()
	{
		...
		broadcastTo(new SystemMessage(SystemMsg.YOU_ARE_PARTICIPATING_IN_THE_SIEGE_OF_S1_THIS_SIEGE_IS_SCHEDULED_FOR_2_HOURS).addResidenceName(getResidence()), ATTACKERS, DEFENDERS);
	...
	public void stopEvent(boolean step)
	{
		...
		if(ownerClan != null)
		{
			if(_oldOwner == ownerClan)
			{
				...
				ownerClan.broadcastToOnlineMembers(new SystemMessage(SystemMsg.SINCE_YOUR_CLAN_EMERGED_VICTORIOUS_FROM_THE_SIEGE_S1_POINTS_HAVE_BEEN_ADDED_TO_YOUR_CLANS_REPUTATION_SCORE).addNumber(ownerClan.incReputation(1500, false, toString())));
			}
			else
			{
				...
				ownerClan.broadcastToOnlineMembers(new SystemMessage(SystemMsg.SINCE_YOUR_CLAN_EMERGED_VICTORIOUS_FROM_THE_SIEGE_S1_POINTS_HAVE_BEEN_ADDED_TO_YOUR_CLANS_REPUTATION_SCORE).addNumber(ownerClan.incReputation(3000, false, toString())));

				if(_oldOwner != null)
					_oldOwner.broadcastToOnlineMembers(new SystemMessage(SystemMsg.YOUR_CLAN_HAS_FAILED_TO_DEFEND_THE_CASTLE_S1_POINTS_HAVE_BEEN_DEDUCTED_FROM_YOU_CLAN_REPUTATION_SCORE_AND_ADDED_TO_YOUR_OPPONENTS).addNumber(-_oldOwner.incReputation(-3000, false, toString())));
	...
	public void removeState(int val)
	{
		...
		if(val == REGISTRATION_STATE)
			broadcastToWorld(new SystemMessage(SystemMsg.THE_REGISTRATION_TERM_FOR_S1_HAS_ENDED).addResidenceName(getResidence()));
	}
	...
	public void generateNextSiegeDates()
	{
		...
		calendar.setTimeInMillis(Config.CASTLE_VALIDATION_DATE.getTimeInMillis());
	...
	private void setNextSiegeTime()
	{
		...
		calendar.setTimeInMillis(Config.CASTLE_VALIDATION_DATE.getTimeInMillis());
		calendar.set(Calendar.DAY_OF_WEEK, Calendar.SUNDAY);
		calendar.set(Calendar.HOUR_OF_DAY, getResidence().getLastSiegeDate().getTimeInMillis() == 0 ? Rnd.get(Config.CASTLE_SELECT_HOURS) : getResidence().getLastSiegeDate().get(Calendar.HOUR_OF_DAY));
		if(calendar.before(Config.CASTLE_VALIDATION_DATE))
			calendar.add(Calendar.WEEK_OF_YEAR, 1);
		validateSiegeDate(calendar, 2);
	...
	public boolean canResurrect(Creature active, Creature target, boolean force, boolean quiet)
	{
		...
		CastleSiegeEvent siegeEvent1 = resurectPlayer.getEvent(CastleSiegeEvent.class);
		CastleSiegeEvent siegeEvent2 = targetPlayer.getEvent(CastleSiegeEvent.class);
