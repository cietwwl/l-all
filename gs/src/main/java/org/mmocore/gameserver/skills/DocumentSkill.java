...
public final class DocumentSkill extends DocumentBase
{
	...
	public class Skill
	{
		...
		public List<org.mmocore.gameserver.model.Skill> skills = new ArrayList<org.mmocore.gameserver.model.Skill>();
		public List<org.mmocore.gameserver.model.Skill> currentSkills = new ArrayList<org.mmocore.gameserver.model.Skill>();
	}

	private Skill currentSkill;
	private Set<String> usedTables = new HashSet<String>();
	private List<org.mmocore.gameserver.model.Skill> skillsInFile = new LinkedList<org.mmocore.gameserver.model.Skill>();
	...
	protected List<org.mmocore.gameserver.model.Skill> getSkills()
	{
		return skillsInFile;
	}
	...
	protected void parseSkill(Node n)
	{
		...
		try
		{
			...
			for(int k = 0; k < d.getChildNodes().getLength(); k++)
			{
				...
				if(eLevels == 0)
				{
					value = enchant.getAttributes().getNamedItem("levels");
					if (value != null)
						eLevels = Integer.parseInt(value.getNodeValue());
					else
						eLevels = 30;
				}
				String ename = enchant.getAttributes().getNamedItem("name").getNodeValue();
				value = enchant.getAttributes().getNamedItem("importance");
				if (value == null)
					throw new RuntimeException("Skill id=" + skillId + " enchant " + ename + " importance is absent !");
				int importance = Integer.parseInt(value.getNodeValue());
				for(int r = 1; r <= eLevels; r++)
				{
					int level = lastLvl + eLevels * count + r;
					EnchantSkillLearn e = new EnchantSkillLearn(skillId, 100 * (count + 1) + r, skillName, "+" + r + " " + ename, (r == 1 ? lastLvl : 100 * (count + 1) + r - 1), lastLvl, eLevels, importance);
			...
			for(int i = 0; i < lastLvl; i++)
			{
				...
				org.mmocore.gameserver.model.Skill current = currentSkill.currentSkills.get(i);
	...
	protected void parseExtract(Node n, StatsSet set)
	{
		List<Extract.ExtractGroup> list = new ArrayList<Extract.ExtractGroup>();
		...
		for(n = n.getFirstChild(); n != null; n = n.getNextSibling())
		{
			if("group".equalsIgnoreCase(n.getNodeName()))
			{
				...
				double chance = Double.parseDouble(map.getNamedItem("chance").getNodeValue());

				Extract.ExtractGroup g = new Extract.ExtractGroup(chance);
				list.add(g);
				...
				for(Node n2 = n.getFirstChild(); n2 != null; n2 = n2.getNextSibling())
				{
					if("extract".equalsIgnoreCase(n2.getNodeName()))
					{
						...
						int itemId = Integer.parseInt(map.getNamedItem("item_id").getNodeValue());
						int count = Integer.parseInt(map.getNamedItem("count").getNodeValue());
	...
	{
		currentSkill.currentSkills = new ArrayList<org.mmocore.gameserver.model.Skill>(currentSkill.sets.length);
