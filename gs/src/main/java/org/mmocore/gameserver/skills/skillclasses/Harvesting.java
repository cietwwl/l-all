	...
	public void useSkill(SkillEntry skillEntry, Creature activeChar, List<Creature> targets)
	{
		...
		for(Creature target : targets)
			if(target != null)
			{
				...
				if(player.isGM())
					player.sendMessage(new CustomMessage("l2p.gameserver.skills.skillclasses.Harvesting.Chance").addNumber((long) SuccessRate));
				...
				player.sendPacket(new SystemMessage(SystemMsg.C1_HARVESTED_S3_S2S).addName(player).addNumber(item.count).addItemName(item.itemId));
				if(player.isInParty())
				{
					SystemMessage smsg = new SystemMessage(SystemMsg.C1_HARVESTED_S3_S2S).addName(player).addNumber(item.count).addItemName(item.itemId);
