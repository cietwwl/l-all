...
public class Recall extends Skill
{
	...
	public boolean checkCondition(SkillEntry skillEntry, Creature activeChar, Creature target, boolean forceUse, boolean dontMove, boolean first)
	{
		...
		{
			if(activeChar.isPlayer())
				activeChar.sendMessage(new CustomMessage("l2p.gameserver.skills.skillclasses.Recall.Here"));
			return false;
		}
	...
	@Override
	public void useSkill(SkillEntry skillEntry, final Creature activeChar, List<Creature> targets)
	{
		for(final Creature target : targets)
			if(target != null)
			{
				...
				{
					activeChar.sendMessage(new CustomMessage("l2p.gameserver.skills.skillclasses.Recall.Festival"));
					continue;
				}
				...
				if(_isItemHandler && _itemConsumeId.length > 0)
				{
					//TODO [VISTALL] переделать ТП на loc параметр
