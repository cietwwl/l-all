			...
			if(target != null)
			{
				if(getPower() > 0 || _absorbAbs > 0) // Если == 0 значит скилл "отключен"
				{
					...
					if(!corpseSkill)
					{
						...
						boolean crit = false;
						if (isMagic())
							damage = Formulas.calcMagicDam(activeChar, target, skillEntry, sps, false);
						else
						{
							...
							if (info.lethal_dmg > 0)
								target.reduceCurrentHp(info.lethal_dmg, activeChar, skillEntry, 0, crit, true, true, false, false, false, false, false);
						}
						...
						target.reduceCurrentHp(damage, activeChar, skillEntry, 0, crit, true, true, false, true, false, false, true);
