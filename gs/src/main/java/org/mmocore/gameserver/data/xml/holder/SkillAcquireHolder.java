...
public final class SkillAcquireHolder extends AbstractHolder
{
	...
	private IntObjectMap<List<SkillLearn>> _normalSkillTree = new HashIntObjectMap<List<SkillLearn>> ();
	private IntObjectMap<List<SkillLearn>> _transferSkillTree = new HashIntObjectMap<List<SkillLearn>>();
	// расовые зависимости
	private IntObjectMap<List<SkillLearn>> _transformationSkillTree = new HashIntObjectMap<List<SkillLearn>>();
	// без зависимостей
	private List<SkillLearn> _fishingSkillTree = new ArrayList<SkillLearn>();
	private List<SkillLearn> _fishingNonDwarfSkillTree = new ArrayList<SkillLearn>();
	...
	public Collection<SkillLearn> getAvailableSkills(Player player, AcquireType type, SubUnit subUnit)
	{
		...
		switch(type)
		{
			...
			case FISHING_NON_DWARF:
				skills = _fishingNonDwarfSkillTree;

				return getAvaliableList(skills, player.getAllSkillsArray(), player.getLevel());
			case CLAN:
				skills = _pledgeSkillTree;
				Collection<SkillEntry> skls = player.getClan().getSkills(); //TODO [VISTALL] придумать другой способ

				return getAvaliableList(skills, skls.toArray(new SkillEntry[skls.size()]), player.getClan().getLevel());
			case SUB_UNIT:
				skills = _subUnitSkillTree;
				Collection<SkillEntry> st = subUnit.getSkills(); //TODO [VISTALL] придумать другой способ
	...
	private boolean isSkillPossible(Collection<SkillLearn> skills, SkillEntry skill)
	{
	...
	public boolean isSkillPossible(Player player, SkillEntry skill)
	{
	...
	public List<SkillLearn> getAllNormalSkillTreeWithForgottenScrolls()
	{
		...
		for(List<SkillLearn> list : _normalSkillTree.values())
	...
	public void addAllFishingNonDwarfLearns(List<SkillLearn> s)
	{
		_fishingNonDwarfSkillTree.addAll(s);
	}
	...
	public void log()
	{
		info("load " + sizeOfMapList(_normalSkillTree) + " normal learns for " + _normalSkillTree.size() + " classes.");
		info("load " + sizeOfMapList(_transferSkillTree) + " transfer learns for " + _transferSkillTree.size() + " classes.");
		//
		info("load " + sizeOfMapList(_transformationSkillTree) + " transformation learns for " + _transformationSkillTree.size() + " races.");
		//
		info("load " + _fishingSkillTree.size() + " fishing learns.");
		info("load " + _fishingNonDwarfSkillTree.size() + " fishing non dwarf learns.");
	...
	private int sizeOfMapList(IntObjectMap<List<SkillLearn>> a)
	{
		int i = 0;
		for(List<SkillLearn> l : a.values())
			i += l.size();
		return i;
	}
}
