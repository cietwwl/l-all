...
import org.mmocore.gameserver.data.xml.holder.AugmentationDataHolder;
...
import org.mmocore.gameserver.templates.augmentation.OptionGroup;
...
public class AugmentationDataParser extends AbstractFileParser<AugmentationDataHolder>
{
	...
	{
		RndSelector<OptionGroup>[] sel = new RndSelector[AugmentationInfo.MAX_AUGMENTATION_COUNT];
		if(warElement == null)
			return null;
		...
		for(Element variantElement : warElement.elements())
		{
			...
			for(Element groupElement : variantElement.elements())
			{
				...
				int chance = (int)(Double.parseDouble(groupElement.attributeValue("chance")) * 10000);
				...
				for(Element optionElement : groupElement.elements())
				{
					...
					int optionChance = (int)(Double.parseDouble(optionElement.attributeValue("chance")) * 10000);
					allSubGroupChance += optionChance;

					optionGroup.addOptionWithChance(optionId, optionChance);
				}

				if(allSubGroupChance != RewardList.MAX_CHANCE && val != 2)  // со второй вариации - мы игнорируем шанс
