...
public class SummonDAO
{
	...
	private static SummonDAO _instance = new SummonDAO();
	...
	public SummonInstance select(Player owner, int skillId)
	{
		Connection con = null;
		...
		try
		{
			...
			if(rset.next())
			{
				NpcTemplate template = NpcHolder.getInstance().getTemplate(rset.getInt("npc_id"));
				if(template == null)
					return null;

				SummonInstance summon = new SummonInstance(IdFactory.getInstance().getNextId(), template, owner, rset.getInt("current_life"), rset.getInt("max_life"), rset.getInt("item_consume_id"), rset.getInt("item_consume_count"), rset.getInt("item_consume_delay"), skillId);
	...
	public void insert(Player player, SummonInstance summon)
	{
		Connection con = null;
		PreparedStatement statement = null;
		try
		{
			con = DatabaseFactory.getInstance().getConnection();
			statement = con.prepareStatement("INSERT INTO summons (current_hp,current_mp,current_life,max_life,item_consume_id, item_consume_count, item_consume_delay, npc_id, exp_penalty, object_id, call_skill_id) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
	...
	public void delete(int objectId, int skillId)
	{
		Connection con = null;
		PreparedStatement statement = null;
		try
