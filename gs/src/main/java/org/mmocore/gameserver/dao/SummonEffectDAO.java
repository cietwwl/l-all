...
public class SummonEffectDAO
{
	...
	private static SummonEffectDAO _instance = new SummonEffectDAO();
	...
	public void select(SummonInstance summon)
	{
		Connection con = null;
	...
	public void delete(int objectId, int callSkillId)
	{
		Connection con = null;
		PreparedStatement statement = null;
		try
	...
	{
		List<Effect> effects = summon.getEffectList().getAllEffects();
		if(effects.isEmpty())
			return;
		...
		try
		{
			...
			int order = 0;
			SqlBatch b = new SqlBatch("INSERT IGNORE INTO `summon_effects` (`object_id`,`call_skill_id`,`skill_id`,`skill_level`,`effect_count`,`effect_cur_time`,`duration`,`order`) VALUES");
