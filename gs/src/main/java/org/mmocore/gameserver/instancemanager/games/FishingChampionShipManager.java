			...
			for(Fisher fisher : _tmpPlayers)
				if(fisher.getName().equalsIgnoreCase(pl.getName()))
				{
					if(fisher.getLength() < len)
					{
						...
						pl.sendMessage(new CustomMessage("l2p.gameserver.instancemanager.games.FishingChampionShipManager.ResultImproveOn"));
						recalculateMinLength();
					}
					return;
				}
			_tmpPlayers.add(new Fisher(pl.getName(), len, 0));
			pl.sendMessage(new CustomMessage("l2p.gameserver.instancemanager.games.FishingChampionShipManager.YouInAPrizeList"));
			recalculateMinLength();
		}
		...
		for(Fisher fisher : _winPlayers)
			if(fisher._name.equalsIgnoreCase(pl.getName()))
				if(fisher.getRewardType() != 2)
				{
					...
					if(rewardCnt > 0)
					{
						SystemMessage smsg = new SystemMessage(SystemMsg.YOU_HAVE_EARNED_S2_S1S).addItemName(Config.ALT_FISH_CHAMPIONSHIP_REWARD_ITEM).addNumber(rewardCnt);
						pl.sendPacket(smsg);
						ItemFunctions.addItem(pl, Config.ALT_FISH_CHAMPIONSHIP_REWARD_ITEM, rewardCnt);
					}
				}
	}
