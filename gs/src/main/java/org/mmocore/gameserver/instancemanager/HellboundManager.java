...
public class HellboundManager
{
			...
			if(ArrayUtils.contains(hbsi.getStages(), getHellboundLevel()))
				try
				{
					...
					for(int i = 0; i < hbsi.getAmount(); i++)
					{
						...
						spawnDat.setParameters(hbsi.getParameters());
				...
				if("list".equalsIgnoreCase(n1.getNodeName()))
					for(Node d1 = n1.getFirstChild(); d1 != null; d1 = d1.getNextSibling())
						if("data".equalsIgnoreCase(d1.getNodeName()))
						{
							...
							for(Node p1 = d1.getFirstChild(); p1 != null; p1 = p1.getNextSibling())
							{
								if("parameters".equalsIgnoreCase(p1.getNodeName()))
								{
									if(parameters.isEmpty())
										parameters = new MultiValueSet<String>();

									String pName = p1.getAttributes().getNamedItem("name").getNodeValue();
									String pValue = p1.getAttributes().getNamedItem("value").getNodeValue();
							...
							HellboundSpawn hbs = new HellboundSpawn(npcId, spawnLoc, count, territory, respawn, respawnRnd, stages, parameters);
							_list.add(hbs);
						}
	...
	public class HellboundSpawn
	{
		...
		public HellboundSpawn(int npcId, Location loc, int amount, Territory spawnTerritory, int respawn, int respawnRnd, int[] stages, MultiValueSet<String> parameters)
		{
			_npcId = npcId;
			_loc = loc;
			_amount = amount;
