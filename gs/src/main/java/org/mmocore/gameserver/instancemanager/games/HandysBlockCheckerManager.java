...
import org.mmocore.gameserver.network.l2.s2c.ExCubeGameChangeTeam;
...
public final class HandysBlockCheckerManager
{
	...
	public boolean addPlayerToArena(Player player, int arenaId)
	{
		...
		synchronized (holder)
		{
			...
			{
				player.sendPacket(new SystemMessage(SystemMsg.YOU_MUST_WAIT_10_SECONDS_BEFORE_ATTEMPTING_TO_REGISTER_AGAIN));
				return false;
			}
	...
	public class ArenaParticipantsHolder
	{
		...
		public void broadCastPacketToTeam(IBroadcastPacket packet)
		{
		...
		private void checkAndShuffle()
		{
			...
			{
				broadCastPacketToTeam(new SystemMessage(SystemMsg.TEAM_MEMBERS_WERE_MODIFIED_BECAUSE_THE_TEAMS_WERE_UNBALANCED));
