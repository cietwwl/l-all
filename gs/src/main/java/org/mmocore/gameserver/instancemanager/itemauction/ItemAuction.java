...
public class ItemAuction
{
	...
	public void registerBid(Player player, long newBid)
	{
		...
		if(newBid > Config.ALT_ITEM_AUCTION_MAX_BID)
		{
			if(Config.ALT_ITEM_AUCTION_MAX_BID == 100000000000L)
				...
				player.sendMessage("Your bid cannot exceed " + Config.ALT_ITEM_AUCTION_MAX_BID);//TODO [G1ta0] custom message
			return;
		}
		...
		synchronized (_auctionBids)
		{
			...
			if(bid == null)
			{
				...
				player.sendPacket(new SystemMessage(SystemMsg.YOU_HAVE_SUBMITTED_A_BID_FOR_THE_AUCTION_OF_S1).addNumber(newBid));
				return;
			}
	...
	public void broadcastToAllBidders(IBroadcastPacket packet)
	{
	...
	public void cancelBid(Player player)
	{
		...
		synchronized (_auctionBids)
		{
			...
			{
				player.sendMessage("You cannot cancel you bid: You have the highest bid.");//TODO [G1ta0] custom message
				return;
			}
