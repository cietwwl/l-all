...
public class LotteryManager
{
	...
	private class finishLottery extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			...
			{
				// There are winners.
				sm = new SystemMessage(SystemMsg.THE_PRIZE_AMOUNT_FOR_THE_WINNER_OF_LOTTERY_S1_IS_S2_ADENA__WE_HAVE_S3_FIRST_PRIZE_WINNERS);
				sm.addNumber(getId());
			...
			else
			{
				...
				sm = new SystemMessage(SystemMsg.THE_PRIZE_AMOUNT_FOR_LUCKY_LOTTERY_S1_IS_S2_ADENA_THERE_WAS_NO_FIRST_PRIZE_WINNER_IN_THIS_DRAWING_THEREFORE_THE_JACKPOT_WILL_BE_ADDED_TO_THE_NEXT_DRAWING);
				sm.addNumber(getId());
				sm.addNumber(getPrize());
				Announcements.getInstance().announceToAll(sm);
			}
