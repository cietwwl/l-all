...
public class RequestExMPCCAcceptJoin extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		{
			request.cancel();
			requestor.sendPacket(new SystemMessage(SystemMsg.C1_HAS_DECLINED_THE_CHANNEL_INVITATION).addString(activeChar.getName()));
			return;
		}
		...
		try
		{
			...
			else
			{
				...
				if(fromItem && !ItemFunctions.deleteItem(requestor, CommandChannel.STRATEGY_GUIDE_ID, 1))
					return;
