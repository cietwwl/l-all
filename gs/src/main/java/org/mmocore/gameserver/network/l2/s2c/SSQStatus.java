				...
				for(int i = 1; i < 4; i++)
				{
					...
					if(Math.max(dawnProp, duskProp) < 10) // печать будет потеряна если занята
					{
						...
						if(curSealOwner == SevenSigns.CABAL_NULL) // печать останется свободной
							writeD(SystemMsg.SINCE_THE_SEAL_WAS_NOT_OWNED_DURING_THE_PREVIOUS_PERIOD_AND_SINCE_LESS_THAN_35_PERCENT_OF_PEOPLE_HAVE_VOTED.id());
						else
							// печать будет освобождена
							writeD(SystemMsg.ALTHOUGH_THE_SEAL_WAS_OWNED_DURING_THE_PREVIOUS_PERIOD_LESS_THAN_10_OF_PEOPLE_HAVE_VOTED.id());
					}
					else if(Math.max(dawnProp, duskProp) < 35) // печать будет сохранена если занята
					{
						...
						if(curSealOwner == SevenSigns.CABAL_NULL) // печать останется свободной
							...
							writeD(SystemMsg.SINCE_THE_SEAL_WAS_OWNED_DURING_THE_PREVIOUS_PERIOD_AND_10_OR_MORE_PEOPLE_HAVE_PARTICIPATED.id());
					}
					else if(dawnProp == duskProp) // ничья, печать не получит никто
					{
						writeC(SevenSigns.CABAL_NULL);
						writeD(SystemMsg.IF_CURRENT_TRENDS_CONTINUE_IT_WILL_END_IN_A_TIE.id());
					}
					else
						// у кого-то есть перевес
					{
						...
						if(sealWinning == curSealOwner) // состояние не изменится
							...
							// состояние изменится
							writeD(SystemMsg.ALTHOUGH_THE_SEAL_WAS_NOT_OWNED_35_OR_MORE_PEOPLE_HAVE_PARTICIPATED.id());
					}
				}
