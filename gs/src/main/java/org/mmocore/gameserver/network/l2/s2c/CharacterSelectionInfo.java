...
public class CharacterSelectionInfo extends L2GameServerPacket
{
	...
	protected final void writeImpl()
	{
		...
		for(int i = 0; i < size; i++)
		{
			...
			writeH(charInfoPackage.getPaperdollAugmentationId(Inventory.PAPERDOLL_RHAND)[0]);
			writeH(charInfoPackage.getPaperdollAugmentationId(Inventory.PAPERDOLL_RHAND)[1]);
	...
	private static CharSelectInfo restoreChar(ResultSet chardata)
	{
		...
		try
		{
			...
			int newVitality = chardata.getInt("vitality") + (int) ((System.currentTimeMillis() - charInfopackage.getLastAccess()) / 15000.);

			charInfopackage.setVitalityPoints(Math.max(Math.min(newVitality, Config.VITALITY_LEVELS[4]), 0));
