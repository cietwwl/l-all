	...
	protected final void writeImpl()
	{
		...
		for(MultiSellEntry ent : _list)
		{
			...
			for(MultiSellIngredient prod : ent.getProduction())
			{
				...
				writeD(prod.getItemAugmentations()[0]); // инкрустация
				writeD(prod.getItemAugmentations()[1]); // инкрустация
			...
			for(MultiSellIngredient i : ingredients)
			{
				...
				writeD(i.getItemAugmentations()[0]); // инкрустация
				writeD(i.getItemAugmentations()[1]); // инкрустация
				writeItemElements(i);
			}
		}
	}
