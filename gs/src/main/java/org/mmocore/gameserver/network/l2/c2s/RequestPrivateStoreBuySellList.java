...
public class RequestPrivateStoreBuySellList extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		catch(ArithmeticException ae)
		{
			//TODO audit
			sellList.clear();
			seller.sendPacket(SystemMsg.INCORRECT_ITEM_COUNT);
			return;
		}
		finally
		{
			try
			{
				...
				for(TradeItem si : sellList)
				{
					...
					Log.LogItem(seller, ItemLog.PrivateStoreSell, item, item.getCount(), si.getOwnersPrice());
					Log.LogItem(buyer, ItemLog.PrivateStoreBuy, item, item.getCount(), si.getOwnersPrice());
