...
import org.mmocore.gameserver.network.l2.s2c.AskJoinPledge;
...
public class RequestJoinPledge extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		{
			activeChar.sendPacket(new SystemMessage(SystemMsg.S1_IS_ALREADY_A_MEMBER_OF_ANOTHER_CLAN).addName(member));
			return;
		}

		if(!member.canJoinClan())
		{
			activeChar.sendPacket(new SystemMessage(SystemMsg.C1_CANNOT_JOIN_THE_CLAN_BECAUSE_ONE_DAY_HAS_NOT_YET_PASSED_SINCE_THEY_LEFT_ANOTHER_CLAN).addName(member));
			return;
		}
