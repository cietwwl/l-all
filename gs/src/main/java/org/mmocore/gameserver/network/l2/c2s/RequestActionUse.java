...
import org.mmocore.gameserver.network.l2.s2c.ExAirShipTeleportList;
import org.mmocore.gameserver.network.l2.s2c.ExAskCoupleAction;
import org.mmocore.gameserver.network.l2.s2c.PrivateStoreManageListBuy;
import org.mmocore.gameserver.network.l2.s2c.PrivateStoreManageListSell;
import org.mmocore.gameserver.network.l2.s2c.RecipeShopManageList;
...
public class RequestActionUse extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(activeChar.getTransformation() != 0 && !(activeChar.getTransformation() >= 312 && activeChar.getTransformation() <= 318) && action.transform > 0)
		{
			activeChar.sendActionFailed();
			return;
		}
		...
		if(action.type == 4)
		{
			...
			{
				activeChar.sendPacket(new SystemMessage(SystemMsg.C1_IS_ALREADY_PARTICIPATING_IN_A_COUPLE_ACTION_AND_CANNOT_BE_REQUESTED_FOR_ANOTHER_COUPLE_ACTION).addName(pcTarget));
				return;
			}
			if(pcTarget.isProcessingRequest())
			{
				activeChar.sendPacket(new SystemMessage(SystemMsg.C1_IS_ON_ANOTHER_TASK).addName(pcTarget));
				return;
			}
			...
			activeChar.sendPacket(new SystemMessage(SystemMsg.YOU_HAVE_REQUESTED_A_COUPLE_ACTION_WITH_C1).addName(pcTarget));
		...
		switch(action.id)
		{
			...
			case 0: // Сесть/встать
				...
				{
					if(target != null && target instanceof StaticObjectInstance && ((StaticObjectInstance) target).getType() == 1 && activeChar.isInRangeZ(target, Creature.INTERACTION_DISTANCE))
			...
			case 38: // Mount
				...
				else if(activeChar.isMounted() || activeChar.isInBoat() || activeChar.isInDuel() || activeChar.isCursedWeaponEquipped() || activeChar.isCastingNow() || activeChar.isParalyzed() || activeChar.getActiveWeaponFlagAttachment() != null || activeChar.isFishing())
				{
					//
				}
				else
	...
	{
		Player activeChar = getClient().getActiveChar();
		Servitor pet = activeChar.getServitor();
		if(pet == null)
		{
			activeChar.sendActionFailed();
			return;
		...
		Creature aimingTarget = skill.getTemplate().getAimingTarget(pet, activeChar.getTarget());
