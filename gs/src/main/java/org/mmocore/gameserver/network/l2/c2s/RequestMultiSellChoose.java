...
public class RequestMultiSellChoose extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		try
		{
					...
					if(!template.isStackable())
						for(int i = 0; i < ingridientItemCount * _amount; i++)
						{
							...
							if(keepenchant)
							{
								...
								if(itemToTake == null)
								{
									if (ingridientItemCount > 1)
										activeChar.sendPacket(new SystemMessage(SystemMsg.S2_UNITS_OF_THE_ITEM_S1_ISARE_REQUIRED).addItemName(ingridientItemId).addNumber(ingridientItemCount));
									else
										activeChar.sendPacket(new SystemMessage(SystemMsg.THE_ITEM_S1_IS_REQUIRED).addItemName(ingridientItemId));
									return;
								}
							...
							else
							{
								ItemInstance itemToTake = null;
								for(ItemInstance item : list)
									if(!items.contains(new ItemData(item.getItemId(), item.getCount(), item)) && (itemToTake == null || item.getEnchantLevel() < itemToTake.getEnchantLevel()) && !item.isEquipped() && !item.isShadowItem() && !item.isTemporalItem() && !item.isAugmented() && ItemFunctions.checkIfCanDiscard(activeChar, item))
									{
										itemToTake = item;
			...
			int enchantLevel = 0, mineralId = 0;
			...
			for(ItemData id : items)
			{
				...
				if(count > 0)
					...
					else
					{
						...
						{
							if(keepenchant && it.canBeEnchanted(true))
							{
								enchantLevel = it.getEnchantLevel();
