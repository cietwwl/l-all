...
import org.mmocore.gameserver.network.l2.s2c.ExReplyRegisterDominion;
...
public class RequestExJoinDominionWar extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(siegeEvent.isRegistrationOver() || dominion.getSiegeDate().getTimeInMillis() < System.currentTimeMillis())
		{
		...
		if(_isRegistration)
		{
			...
			if(_clanRegistration)
			{
				...
				player.sendPacket(new SystemMessage(SystemMsg.CLAN_PARTICIPATION_IS_REQUESTED_IN_S1_TERRITORY).addResidenceName(dominion));
			}
			else
			{
				...
				player.sendPacket(new SystemMessage(SystemMsg.MERCENARY_PARTICIPATION_IS_REQUESTED_IN_S1_TERRITORY).addResidenceName(dominion));
			}
		}
		else
		{
			...
			if(_clanRegistration)
			{
				...
				player.sendPacket(new SystemMessage(SystemMsg.CLAN_PARTICIPATION_REQUEST_IS_CANCELLED_IN_S1_TERRITORY).addResidenceName(dominion));
			}
			else
			{
				...
				player.sendPacket(new SystemMessage(SystemMsg.MERCENARY_PARTICIPATION_REQUEST_IS_CANCELLED_IN_S1_TERRITORY).addResidenceName(dominion));
			}
		}
