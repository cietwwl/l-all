...
import org.mmocore.gameserver.network.l2.s2c.ExPCCafePointInfo;
...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		if(fullString.equals("admin_current_player"))
			...
			try
			{
				int newVal = Integer.parseInt(wordList[1]);
				GameObject target = activeChar.getTarget();
				if(target == null)
				{
				...
				if(newVal >= 0)
				{
					int old = player.getPkKills();
					player.setPkKills(newVal);
					player.sendChanges();

					player.sendMessage("Admin has changed your pk kills from " + old + " to " + newVal + ".");
					activeChar.sendMessage("Successfully Changed pk kills for " + player.getName() + " from (" + old + ") to (" + newVal + ").");
				}
				else
					activeChar.sendMessage("You must enter a value for pk kills greater than or equal to 0.");
			}
			catch(ArrayIndexOutOfBoundsException e)
			{
				activeChar.sendMessage("Please specify new pk kills value.");
			}
			...
			try
			{
				...
				if(newVal >= 0)
				{
					int old = player.getPvpKills();
					player.setPvpKills(newVal);
					player.sendChanges();

					player.sendMessage("Admin has changed your pvp kills from " + old + " to " + newVal + ".");
					activeChar.sendMessage("Successfully Changed pvp kills for " + player.getName() + " from (" + old + ") to (" + newVal + ").");
				}
				else
					activeChar.sendMessage("You must enter a value for pvp kills greater than or equal to 0.");
			}
			catch(ArrayIndexOutOfBoundsException e)
			{
				activeChar.sendMessage("Please specify new pk kills value.");
		...
		else if(fullString.startsWith("admin_setheroaura"))
		{
			GameObject target = activeChar.getTarget();
			Player player;
			...
			if (player.isHero())
			{
				activeChar.sendMessage("You can't change aura status of hero character.");
				return false;
			}

			if(player.isHeroAura())
			{
				player.setHeroAura(false);
				player.unsetVar(Player.HERO_AURA);
			}
			else
			{
				long time = 0;
				try
				{
					if (wordList.length > 2 && wordList[2] != null)
						time = System.currentTimeMillis() + 24 * 60 * 60 * 1000 * Integer.parseInt(wordList[2]);
				}
				catch(NumberFormatException e)
				{
					activeChar.sendMessage("Command format is //setheroaura <player> <days>");
					return false;
				}

				player.setVar(Player.HERO_AURA, 1, time);
		...
		else if(fullString.startsWith("admin_setsubclass"))
		{
			...
			if(st.countTokens() > 1)
			{
				...
				{
					activeChar.sendMessage(new CustomMessage("l2p.gameserver.model.instances.L2VillageMasterInstance.SubclassCouldNotBeAdded"));
					return false;
				}
		...
		else if(fullString.startsWith("admin_give_item"))
		{
			...
			ItemFunctions.addItem(activeChar.getTarget().getPlayer(), id, count);
		}
	...
	{
		Collection<Player> p0 = GameObjectsStorage.getPlayers();
		Player[] players = p0.toArray(new Player[p0.size()]);
	...
	private void setSubclass(final Player activeChar, final Player player)
	{
		...
		{
			activeChar.sendMessage(new CustomMessage("l2p.gameserver.model.instances.L2VillageMasterInstance.NoSubAtThisTime"));
			return;
		}
